...
def make_default_config():
    ...
    os.makedirs(os.path.expandvars("/app/config"), exist_ok=True)
    with open(os.path.expandvars("/app/config/config.json"), "w", encoding='utf-8') as output:
        json.dump(new_config, output, indent=2, ensure_ascii=False)
    input(f"Default config file has been generated at {os.path.expandvars('/app/config/config.json')}, please check and modify it now. Please enter to continue...")


def get_config() -> dict:
    if not os.path.exists(os.path.expandvars("/app/config/config.json")):
        print("Configuration file does not exist, creating default settings in [Document library folder]")
        make_default_config()
    with open(os.path.expandvars("/app/config/config.json"), "r", encoding='utf-8') as f:
